...
from biolm.predictors.utils import _configure_optimizers#, data_dir
...
def get_data_loader(data_dir, ddp, batch_size, num_workers, tok_mode='char', **kwargs):
    ...
    train_dataset_0 = PADataset(df, mode='train', fold=fold, crop_size=crop_size, tok_mode=tok_mode)

    df = pd.read_csv(f'{data_dir}/siRNA/agt_10_percent.csv')
    train_dataset_1 = PADataset(df, mode='train', fold=fold, crop_size=crop_size, tok_mode=tok_mode)
    train_dataset = train_dataset_0 + train_dataset_1
    df = pd.read_csv(f'{data_dir}/siRNA/siRNA_TE249.csv')
    valid_dataset = PADataset(df, mode='valid', fold=fold, crop_size=crop_size, tok_mode=tok_mode)
    ...
    df = pd.read_csv(f'{data_dir}/siRNA/agt_90_percent.csv')
    test_dataset = PADataset(df, mode='test', fold=fold, crop_size=crop_size, tok_mode=tok_mode)
...
if __name__ == '__main__':
    ...
    train_sampler, train_loader, valid_loader, test_loader = \
        get_data_loader(data_dir, ddp, batch_size, num_workers, tok_mode='MarsTok')

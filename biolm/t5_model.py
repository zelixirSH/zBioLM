...
class MyT5(nn.Module):
    ...
    def forward(
        ...
        if labels is not None:
            ...
            self.last_loss = loss_fct(lm_logits.view(-1, lm_logits.size(-1)), labels.view(-1))
...
class MyT5_extractor(nn.Module):
    def __init__(self, config: T5Config):
        super().__init__()

        self.t5_encoder = MyT5(config).encoder
        self.params = config
        self.params.dim = config.d_model

    def forward(self,
                tokens: torch.Tensor,
                attn_mask: torch.Tensor,
                targets: Optional[torch.Tensor] = None) -> (torch.Tensor, torch.Tensor):
        '''
        :param tokens: (bsz, seqlen) LongTensor of input token indices
        :param attn_mask: (bsz, seqlen) padding mask for attention
        '''

        encoder_outputs = self.t5_encoder(input_ids=tokens, attention_mask=attn_mask)

        hidden_states = encoder_outputs.hidden_states

        return None, hidden_states

def load_model(ckpt_path, config_path, device = 'cpu', dropout = None, layer_dropout = None, from_scratch = False):
    ...
    # hard code
    if ckpt_path is not None and 'ul2' in ckpt_path:
        config.vocab_size += 1

    model = MyT5_extractor(config)

    if from_scratch:
        print(f'number of encoder parameters: {sum(p.numel() for p in model.t5_encoder.parameters())}')
        return model
    ...
    # get all encoder parameters from the checkpoint
    encoder_state_dict = {k.replace('encoder.',''): v for k, v in state_dict.items() if k.startswith('encoder')}
    model.t5_encoder.load_state_dict(encoder_state_dict, strict=True)
    print(f'number of encoder parameters: {sum(p.numel() for p in model.t5_encoder.parameters())}')
    return model

if __name__ == '__main__':
    config_path = '/public/home/taoshen/code/Projects/Zero2Hero/llama2/config/t5/t5_config_demo.json'
    ckpt_path = '/public/home/taoshen/code/Projects/Zero2Hero/llama2/out_ckpts/mars/mars_run-t5-mars-s-train-val-d0.15-2023_10_21_22_25_05/ckpt.pt'
    load_model(ckpt_path, config_path)

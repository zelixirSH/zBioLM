...
# model mode
parser.add_argument("--model_mode", type=str, default='encoder') # encoder / decoder
parser.add_argument("--data_type", type=str, default='Protein')  # NA / Protein
parser.add_argument("--data_mode", type=str, default='MLM' )     # GLM / MLM
# moe config
...
parser.add_argument("--db", type=str, default='uniref5090')
...
max_step = args.max_step * 1000  # 175
...
assert model_mode in ['encoder', 'decoder']
ss_mask_mode = 'v1'
...
db = 'uniref5090'
train_data_config, val_data_config = get_data_config(db, ss_mlm_prob=mlm_prob)

# db = 'default'
# train_data_config = None
# val_data_config = None
# if args.train_data_dir:
#     train_data_config = [ (args.train_data_dir, "train_char", 1.0)]
# if args.val_data_dir:
#     val_data_config = [ (args.val_data_dir, "train_char", 1.0)]
...
def create_dataloader(
    ...
    for data_dir, prefix, _ in data_config:
        filenames = sorted(glob.glob(str( Path(data_dir) / f"{prefix}*")))
        assert len(filenames) > 0, f"no files found in {data_dir} with prefix {prefix}"
